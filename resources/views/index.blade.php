...
<div class="l_index__container">
    ...
    <div class="l_index__head">
        ...
        <form method="GET" id="js-toggle-display" class="c_serch__container u_display-hide u_font__default"
            action="{{ route('index' , [ $currentPageNum, $_GET['sort'] , $_GET['order'] ]) }}">
            ...
            <div class="l_index__paginate">
                <ul  class="l_pagination__container u_display--center">
                    ...
                    @endphp
                        ...
                        {{-- 現在のページ数が１以外の時表示する --}}
                        @if($currentPageNum != 1)
                                <a class="c_pagination__list--prev c_pagination__list--text u_font__default" href="{{route('index' , [1 , $sort , $order])}}">
                                    <i class="fas fa-chevron-left"></i>
                                </a>
                        @endif
            
                        @for($i = $minPageNum; $i <= $maxPageNum; $i++)
                                <a  class="c_pagination__list--icon <?php if(($currentPageNum ) == $i ) echo "c_pagination--active" ?> c_pagination__list--text u_font__default" href="{{route('index' ,[ $i , $sort , $order])}}">
                                    <?php echo $i ?>
                                </a>
                        @endfor
                        ...
                        @if ($currentPageNum != $maxPageNum)
                                <a  class="c_pagination__list--text u_font__default c_pagination__list--next" href="{{route('index' , [$maxPageNum , $sort , $order])}}">
