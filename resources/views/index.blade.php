        ...
        <form method="GET" id="js-toggle-display" class="c_serch__container u_display-hide u_font__default"
            ...
            <div class="c_serch--sort">
                ...
                <select name="sort" class="c_serch--select">
                    <option value=""         @if($sort == "id")       selected @endif>-ソート-</option>
            ...
            <div class="c_serch--order">
                <select name="order" class="c_serch--select">
                    <option value=""     @if($sort == "")      selected @endif>-昇順・降順-</option>
            ...
            <div class="l_index__paginate">
                <ul  class="l_pagination__container u_display--center">
                    ...
                    @endphp
                        ...
                        {{-- 現在のページ数が１以外の時表示する --}}
                        @if($currentPageNum != 1)
                                <a class="c_pagination__list--prev c_pagination__list--text" href="{{route('index' , [1 , $sort , $order , $expiration, $prefectural ])}}">
                                    <i class="fas fa-chevron-left"></i>
                                </a>
                        @endif
            
                        @for($i = $minPageNum; $i <= $maxPageNum; $i++)
                                <a  class="c_pagination__list--icon <?php if(($currentPageNum ) == $i ) echo "c_pagination--active" ?> c_pagination__list--text" href="{{route('index' ,[ $i ,$sort , $order , $expiration, $prefectural ])}}">
                                    <?php echo $i ?>
                                </a>
                        @endfor
                        ...
                        @if ($currentPageNum != $maxPageNum)
                                <a  class="c_pagination__list--text c_pagination__list--next" href="{{route('index' , [$maxPageNum , $sort , $order , $expiration, $prefectural ])}}">
