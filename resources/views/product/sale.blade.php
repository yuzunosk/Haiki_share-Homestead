        ...
        <div class="l_index__paginate">
          <ul  class="l_pagination__container u_display--center">
              ...
              @endphp
                  ...
                  {{-- 現在のページ数が１以外の時表示する --}}
                  @if($currentPageNum != 1)
                          <a class="c_pagination__list--prev c_pagination__list--text" href="{{route('store.product.sale' , 1)}}">
                              <i class="fas fa-chevron-left"></i>
                          </a>
                  @endif
      
                  @for($i = $minPageNum; $i <= $maxPageNum; $i++)
                          <a  class="c_pagination__list--icon <?php if(($currentPageNum ) == $i ) echo "c_pagination--active" ?> c_pagination__list--text" href="{{route('store.product.sale' , $i )}}">
                              <?php echo $i ?>
                          </a>
                  @endfor
                  ...
                  @if ($currentPageNum != $maxPageNum)
                          <a  class="c_pagination__list--text c_pagination__list--next" href="{{route('store.product.sale' , $maxPageNum )}}">
