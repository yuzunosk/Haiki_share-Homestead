...
class Store_PassRemindRecieveController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($auth_key !== $request->session()->get('auth_key')) {
            return view('store.auth.passwords.store-reset')->with('flash_message', '認証キーが合いません、正しく入力してください');
        }
        Log::info('認証キーの照合が合っています。');

        if (time() > $request->session()->get('auth_key_limit')) {
            Log::info('セッションデータの期限:' . $request->session()->get('auth_key_limit'));
            return redirect()->route('store.password.request')->with('flash_message', '認証キーの期限が切れています。再度認証キー取得を行ってください。');
        }

        $storedata = Store::where("email", $request->session()->get('email'))->first();
        Log::info("ユーザー情報:" . $storedata);
        ...
        return redirect()->route('store.login')->with('flash_message', 'パスワードは正しく変更されました');
    }
}
