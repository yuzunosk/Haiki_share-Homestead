...
class Store_PassRemindSendController extends Controller
{
    ...
    public function show()
    {
        ...
        return view('store.auth.passwords.store-email');
    }
    ...
    public function resetEmail(Request $request)
    {
        ...
        // レコードがあるかどうか判定
        $res = Store::where("email", $request->email)->count();
        Log::info($res);
        ...
        if ($res) {
            ...
            return redirect()->route('store.password.request')->with('flash_message', __('このEmailアドレスは登録されておりません.'));
        }

        return redirect()->route('store.password.reset', compact('token'))->with('flash_message', __('入力されたEmailアドレスへメールを送信いたしました.'));
    }
}
